...
{{< video https://youtu.be/8uRcB34Hhsw?si=AlSxeWoMzx0g9QlL width="100%" height="75%" >}}
...
<br><br>
- **DuckDB**: A lightweight, "in-process" SQL database that executes analytical queries very fast right inside your code - No separate server is needed and you can initialized in-line
<br><br>
...
- The results show the ETL pipeline [on average summarized 1.1 billion rows in 5.04s]{.fragment .highlight-green}, benchmarked over 100 iterations
...
-   DuckDB runtime was [0.05 seconds]{.fragment .highlight-green}
...
-   The code below shows the basic transformation done to the data using `dplyr`, `arrow`, `duckdb`, `duckplyr`, and `polars`
...
nyc_taxi |> 
    ...
    dplyr::collect()
```
 - The ending `collect()` writes the lazy / scanned data back to a df
